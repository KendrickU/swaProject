...
A man in the middle attack can be done during the certificate creation and the time it is passed to the server. During this opening if the man in the middle attack successed the disgruntled user can obtain the certficate and impersonate the bank.
...
Staple-ocsp
Enables OCSP Stapling. A valid OSCP response is stapled to the certificate that the server offers during TLS.
Strict-permissions
...
* Certificate revocation
* Storing a certificate in the filesystem
* Certificate validation




** Certbot is a command line utility,

5 essential dataflows of your project (5 legitimate things they do)
Creating an account (admin)
Creating certificates for valid site’s (admin)
Validate  the certificate to see if it has expired (admin)
Write certificate to the filesystem (admin)
Revoke certificates (admin)

5 threat actors (5 things attackers they do)
Stealing an account
Creating certificates to impersonate a site’s identity
This scenario:
...
5 misuse cases (Shows what allows an attack to happen):
